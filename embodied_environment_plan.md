...
#### 6.2.6b 掩体 / 工具阶梯 / 远程压制（2026-08-19）

驱动问题（用户观察）：玩家① 不会建庇护所抵御怪物、② 不会一步步把工具升级成石制、
③ 怪物难度过高经常被杀。用 `scripts/diag_rollout.py` 跑固定 (任务, seed) 并抓死亡前
25 步轨迹后，定位到三条根因——**都不在任务图，而在原语技能层与制备策略**：

1. **弓的远距射击是死代码**（`_bow_combat`）。直线分支写
   `DELTA_TO_ACTION.get((dx, dy))`，而 `DELTA_TO_ACTION`（`path_planner.py`）只有 4 个
   **单位**向量键 → `dist>1` 时恒为 `None` 并 `continue`。于是弓**只在贴脸（dist==1）
   时才射**，§6.2.6a 记载的"提前射杀免接战 = 弓的核心价值"从未真正运行。
   实测后果：L1 死亡轨迹 hp 10→7→4→1，**每一跳都发生在 `adj=0`（无近身怪）时**——
   兽人法师在 4-5 格外开火（`_move_ranged_mob`），玩家带着 13-18 支箭被点死。
   修复：取符号向量 `(sign(dx), sign(dy))`。
2. **庇护所是彻底缺失的原语**。`PLACE_STONE` 在执行器里只作为 `native.place_stone`
   任务出现，从未当战术用。而机制上掩体极强（`game_logic.py` 核实）：
   - 近战怪只在**曼哈顿距离 ==1** 时攻击（`_move_melee_mob`），移动受 collision 阻挡
     → 三面墙的坑位把同时可接战的怪从 4 只降到 1 只；
   - 怪的投射物撞到 solid 方块即消失（`_move_mob_projectile` 的 `in_wall`，水例外）
     → 墙挡箭；
   - 怪只在距玩家 >9 且 <14 的环上刷新、75% 概率朝玩家走（`spawn_mobs`）
     → "在坑位里等怪上门"是有效战术，且比在旷野迎上去每只少挨一次首击。
   **一个决定实现形态的机制约束**：`place_block` 落在**朝向格**，而 `move_player`
   "能走就走、走不动才只转向" → 只能封住行进方向正前方的格。要面向某个开口就必须
   先走进它，因此**四面自封在 Craftax 里做不到**；最后一面开口无法在原地封上。
   可行的最好结果是"开口 1 个"的坑位：天然凹陷，或用镐向石堆里挖一格
   （挖出的格三面仍是石头）。放石只用于把 2 面墙的角落补成 3 面。
   落地：`craftax/planner/shelter.py`（纯函数几何：`wall_count` / `exposed_dirs` /
   `find_cover_tile` / `find_dig_pocket` / `seal_target`）+ 执行器 `_take_cover`
   （找坑位 → 挖坑位 → 放石封口三级回退），接入远程压制、睡眠、防御性回血、
   夜间驻守、清怪等待五处。
3. **工具阶梯被"有弓就跳过深制备"吃掉**。石镐挂在
   `sword_target >= 3 or not has_bow` 下，而弹药经济学（§6.2.6a）通常给
   `sword_target=2` → 木镐锁死 → 采不到铁 → 铁剑/铁甲/钻石整条链断掉。
   实测 **8/8 局死亡时 `pickaxe` 恒为 1**。修复：镐阶梯与择优解耦，按 `_max_floor`
   逐级升（L1→木镐、L2+→石镐、L4+→铁镐），并把 `pickaxe` 纳入 `FLOOR_GEAR_REQ`
   就绪门（弓可豁免剑/甲，但豁免不了镐——弓解决"怎么打"，镐解决"能采到什么"）。
   顺序刻意是"剑 → 镐"：两者同价（1 木 + 1 石），但剑是**当下**的清怪减伤、
   镐是**后续**采矿链的前置。

夜间策略（新增）：L0 近战刷新率 `0.02 + 0.1×(1-light)²`（`FLOOR_MOB_SPAWN_CHANCE`
的夜间项），天黑时 0.02 → 0.12/步（未清层再 ×3）。按"条件驻守"实现：仅当
L0 + 夜间 + (血<8 或 能量<5) 时进掩体睡到天亮，并照抄 `_regen_mode_active` 的
滞回 + 步数预算（120）+ 冷却（200）——"天黑就躲"会吃掉一局约 30% 的步数。

**修弓引出的新失效模式（同轮修掉）**：箭道打通后第一版 A/B 反而更差——L0 击杀
17→29、箭耗到 0、一次没下楼、成就 18→17。原因是主动防御按 14 格无差别点射，而
**已清层（L0 初始 `monsters_killed=10`）的击杀买不到任何东西**，地表又持续刷新 →
整局变成"地表箭工厂"（§6.2.6a 记载过同名失效，这次由另一条路径复现）。修法是把
交战半径按"击杀是否计入门槛"区分：未清层 14 格（每杀都算下楼门槛），已清层
`CLEARED_FLOOR_ENGAGE_DIST=6`（只打够得着我们的怪，远程射程 5 + 1 格余量）。

任务图侧只做加法：新增 `native.build_shelter`（composite，
`COLLECT_STONE ∧ PLACE_STONE`，依赖 `craft_wood_pickaxe` / `collect_stone` /
`place_stone`），走既有生成流程（`task_graph_agents/exploration_proposals.json` →
`scripts/build_task_graph.py merge`）。现有 77 个任务的谓词与依赖边一字未动，
已录制数据集的标签口径不受影响。节点 92→93、边 179→182。
（同时发现 `craftax/tasks/task_graph.json` 相对 `builtin/*.py` 是**过期产物**——
§6.2.6a 修过的因果边没有重新生成，本轮 merge 一并对齐。）

**A/B 实测**（`scripts/diag_rollout.py`，12 局 = 4 任务 × golden seeds 2011/2111/3017/3050，
每局 2000 步预算；`旧` = 本轮改动前的 HEAD，两侧都带 `data/seed_scan.json`）：

| 指标 | 旧 | 新·掩体一律 3 面墙 | **新·远程压制降到 2 面墙（采用）** |
|---|---|---|---|
| 死亡 | 9/12 | 5/12 | 8/12 |
| 任务完成 | 1 | 2 | 2 |
| 平均存活步数 | 863 | 1481 | 1187 |
| 平均成就数 | 14.9 | 17.9 | **18.8** |
| 远程受击 / 千步 | 4.15 | 4.16 | **2.95**（−29%） |
| 近战受击 / 千步 | 9.27 | 5.51 | 6.88（−26%） |
| **L1 击杀合计** | 3 | 4 | **15**（5×） |
| 达到石镐及以上 | 2/12 | 8/12 | 7/12 |

两个"新"列的差别只有一处：远程压制下找掩体的门槛（3 面墙 / 8 步 → 2 面墙 / 12 步，
即 `RANGED_COVER_STEPS` 与该分支的 `min_walls`）。取 2 面墙的理由是**挡住射线就够**：
一个墙角已封掉两个方向的箭道，而地表 8 步内往往不存在 3 面墙的坑位——按 3 面墙找时
远程受击率完全没降（4.15→4.16），按 2 面墙才降到 2.95。

选型取舍（明确记录，便于回退）：3 面墙那版**活得更久但停在 L0**（L1 击杀 4，
和基线的 3 几乎一样），而 §6.2.6a 已经把"活着但从未清怪"列为失效模式；2 面墙那版
死亡数回升到 8/12，但**这是"真的下去打了"的代价**——L1 击杀 3→15、成就 14.9→18.8、
两个任务完成。若后续把目标改成"最大化存活时长"，把该分支的 `min_walls` 调回
`MIN_SHELTER_WALLS` 即可切换回另一档。

仍未闭合（诚实记录）：
- **L1 清怪墙仍未突破 8 杀**：单局最好 6 杀（seed 3017），距下 L2 的 `monsters_killed>=8`
  还差一点。剩下的杠杆是文档 6.2.6a 出路 c)（路线规划减少 L0/L1 走动暴露）
  与弓的 `hits_per_kill` 标定。
- **旷野无掩体时依然会死**：实测死亡轨迹多为"低血 + 缺水 + 夜间 + 8-12 步内没有任何
  可用墙面"，此时 `_take_cover` 返回 None，只能继续走去找水。掩体是地形依赖的技能，
  在纯草原上无解——真正的解法是提前在水源附近挖好坑位（尚未实现的"基地"概念）。
- `collect_iron` 在 seed 3017 上两侧都在 122 步死亡（目标停在 `craft_stone_pickaxe`），
  与本轮改动无关，是 L0 出生点资源分布问题。

#### 6.2.6c 口渴衰减参数化 + 三个"等待"缺陷（2026-08-19 第二轮）

**口渴衰减做成环境参数**（`EnvParams.thirst_rate`，1.0 = 原版）。动机是长程任务：
原版清醒每 ~21 步掉 1 点水，满水 9 点 ≈ 190 步，2000 步的任务要被"去找水"打断
十余次，而找水路上的暴露又是主要死因（实测死亡轨迹里"低血 + 缺水 + 夜间"最常见）。

- `game_logic.update_player_intrinsics(state, action, params, static_params)` 现在把
  `params.thirst_rate` 乘在口渴累加速率上——"每掉 1 点水的步数" = `21 / thirst_rate`，
  线性可预期。
- **`EnvParams()` 默认仍是 1.0**：RL 基准/既有训练脚本的动力学一字不变。
- 具身层（`SessionActor`）默认按 `contracts.DEFAULT_THIRST_RATE = 0.15` 覆盖
  （≈ 140 步/点，满水 ≈ 1260 步），可由 `POST /sessions` 的 `thirst_rate` 显式改回 1.0。
- **planner 侧必须同步**：`combat_model.project_sleep` / `projected_awake_health` 增加
  `thirst_rate` 参数，`SkillChainExecutor(thirst_rate=...)` 转发。否则会话把水调慢后
  执行器仍按原版投影，会拒绝本来安全的睡眠（"渴着睡"的保护退化成"永远不睡"）。

**把水松开后暴露出三个原本被补给循环掩盖的缺陷**（都由 rollout 轨迹定位）：

1. **满血也会在旷野睡觉**（`_wait_action` 只看血量，不看怪）。实测 seed 2011 夜里
   "血足 → 睡等刷怪"，僵尸走到身边一击 10→3（2 伤 ×**3.5 睡眠倍率**），253 步暴毙。
   水一放松，补给中断变少、待机变多，这条路径被走得更频繁。
   现在 SLEEP 要求"距怪 >=14（怪会消失）**或**身处三面墙坑位"。
2. **原地待命会把自己的掩体挖掉**（本轮新引入的风险）。`DO` 作用于朝向格，而走进
   坑位时正面朝墙 → 每次"低血原地回血"的等待都在挖墙，掩体当场作废。新增
   `_idle_action`：坑位里朝向墙时改用 `NOOP`，朝向水/泉仍用 `DO`（那是喝水）。
3. **坑位里还在追怪**：清怪层站在坑位里却因为"没有直线目标"走出去追击，等于把
   "只有一个开口"的优势还回去。新增伏击分支：清怪层 + 三面墙坑位 + 有箭 →
   `chase="none"`，守住开口等怪上门（怪 75% 概率朝玩家走、只在 10-14 格环刷新）。

**口渴倍率的三档实测**（16 局成对：`enter_gnomish_mines` / `reach_floor_3` ×
8 个 seed，2000 步预算，除 `thirst_rate` 外代码完全相同）：

| `thirst_rate` | 死亡 | 任务完成 | 平均步数 | 平均成就 | L1 击杀合计 | 有 L1 击杀的局 |
|---|---|---|---|---|---|---|
| 1.0（原版） | 10/16 | 0 | 1192 | 19.6 | 23 | 9/16 |
| 0.5 | **6/16** | 0 | **1455** | 19.4 | 19 | 5/16 |
| **0.25（历史具身默认）** | 8/16 | **1** | 1297 | 18.6 | 12 | 2/16 |

读法（结论不唯一，取舍写明便于改）：
- 0.25 给出了**本轮唯一一次真正通关**——`enter_gnomish_mines` seed 2027，1467 步，
  在 L1 清掉 9 只怪后下到 L2。这是 §6.2.7 里"L1 及更深的清怪任务"第一次达成。
- 但 0.25 的**总体推进反而变弱**（L1 击杀 23→12、有 L1 击杀的局 9→2）：水不再逼着
  玩家动，它就更愿意留在地表打转。0.5 是"生存最好、成就与原版持平"的一档。
- 当前具身默认取 0.15（长程任务优先）；改成 0.5 或 1.0 只需
  `contracts.DEFAULT_THIRST_RATE` 一个常量，或按会话传 `thirst_rate`。

**便携水与地表水源（2026-08-19）**：具身层把饮水拆成“水源交互”和“携水补给”两种
动作。玩家可在水/泉旁用 `FILL_WATER` 装满 6 瓶，每瓶恢复 4 点 drink，用
`DRINK_WATER` 在无水楼层补水；满水时不会误消耗瓶子。地表生成会在出生点 4--18 格环
内保证至少一个草地水点，并以 2.5% 种子率生成 3×3 小水塘，且不覆盖道路、树木、矿石
或出生格。执行器把装满便携水作为复合生存任务的中间目标，并在下楼门中按“当前 drink
＋携水恢复量”计算储备，减少往返找水和对单一水源的依赖。便携水库存同时写入服务摘要、
文本观测和符号观测，录制数据可复盘每次补水决策。

**被实测否掉并已回退的改动**（记录以免重复）：把"下楼前值得专程采料"的弹药门槛
从 `arrows_min`（清满 8 只的预算）降到"一个批次"（10 支）。动机来自决策来源打点
（30% 的步耗在 `_collect_resource` 的走路上、背包 wood/stone 恒为 0）。16 局成对
A/B：死亡 8→7、成就 18.6→18.8、L1 击杀 12→12，但**唯一那次通关没了**
（done 1467 步 → timeout）。净收益为负，已回退。

**仍未闭合**：
- L1 清怪墙只在 1/16 的局里被跨过；地表制备仍占掉约 30% 的步数。

**数据集自描述（本轮补齐）**：`shard_manifest.json` 新增 `env_params` 字段，写入
该 shard 实际生效的 `EnvParams` 全量快照（`thirst_rate` / `day_length` / `god_mode`
/ `mob_despawn_distance` / …）。链路是
`SessionActor`（`env_params_to_dict(self._params)`）→ `RecordingConfig.env_params`
→ `AsyncRecorder` → `ShardWriter` → `build_shard_manifest`。逐字段导出而非白名单，
以后往 `EnvParams` 加参数会自动被记录。
**为什么必须记**：动力学不同的 episode 在 transition 层面完全同构——一批数据里混着
`thirst_rate=0.25` 与 `1.0` 的 episode，事后无法筛分，world model 学到的是两套物理
的平均值。

#### 6.2.6d 推进保障：链推进预算 + 储备修复（2026-08-20）

**问题定位**（`native.reach_floor_3`/seed 3017 逐步轨迹，1615 步）：视觉上"一直停在
熔炉附近不推进"，实测不是熔炉——`PLACE_FURNACE`/`MAKE_IRON_SWORD`/`ENTER_DUNGEON`/
`FIND_BOW` 全部完成。真相是 1539/1619 步在 L0，楼层三次触到 1 又退回，
三段停滞窗口（68/58/58 步）玩家都在水坑掩体里 `is_sleeping=True` 回血——
而 `game_logic.craftax_step` 在睡眠中把动作改写成 NOOP，所以规划器发出的 43 次 UP
一次都没生效，人物一格不动。

审查后确认**没有任何机制在度量"任务链是否推进"**，五个缺口：

1. `_progress_signature` 含**位置与背包** → 走一步、捡一根木头就重置停滞计数，
   "在浅层来回采集"永远判成有进展（856 步无楼层推进而 `_stall_steps` 恒为 0）。
   既有的上下楼振荡检测就是这个缺口的特例补丁。
2. `safe_to_progress` 是裸阈值（hp>=6）——与 `_regen_mode_active` 自己记录的教训
   矛盾。地表刷怪使血量在 6 附近震荡时整条下楼路径被无限期关闭。
3. **回血预算漏计睡眠步数**：`_survival_action` 在睡眠时早退，走不到
   `_regen_steps += 1`，于是 68 步睡眠只消耗 0 预算，`REGEN_MAX_STEPS=150`
   实际只在数"走去掩体的那几步"。
4. **冷却只在预算耗尽时设置**：正常回满退出不设冷却 → "下楼→挨打→回血→下楼"
   的**次数**无上限（一局 3 轮，占 53% 步数）。
5. `self.max_steps` 存下来**从未被读取**，且没有任何调用方传过它 → 不存在
   "预算将尽就降低标准去推进"的升级路径。

**修法**：新增链推进预算（签名只含楼层/`chain_idx`/成就数，刻意排除位置与背包），
无推进达 `CHAIN_PUSH_STEPS=250` 进入**推进优先模式**——关掉回血/夜间驻守/补箭/
已清层主动点射，并把下楼血线门从 8 降到 6。推进优先是**脉冲**（`PUSH_MAX_STEPS=150`
+ 250 步冷却）而非常开；`safe_to_progress` 加 latch（进入 6 血、退出 4 血）；
睡眠步数计入恢复类预算；回血正常退出也设 60 步冷却；`max_steps` 默认改为
`estimate_steps()` 并接入 `_budget_is_tight`。

储备侧两处：**带石下楼**（`_arrow_feedstock_stone_target`）——`MAKE_ARROW` 要
1 木 + 1 石，而实测三次下楼都是 8 木 + 0 石，弓在 L1 打空后彻底失去补给能力，
"无箭的弓不算装备"的弹药门也因此永远判定"补不了"而放行；补箭目标改回战斗模型
算出的真实值（L1 约 23 支），不再截到 `BOW_ARROW_RESERVE`。

**被实测否掉的两个设计（勿重复）**：

- **`CHAIN_ABORT_STEPS=600`（无链推进就放弃 seed）**。`collect_diamond` 逐 seed
  实测最长无推进间隔为 2026:210 / 2027:225 / 2028:129 / 2011:189 / 2111:1032 /
  **3017:3541（唯一通关）**——深挖任务本来就有几千步"楼层/子目标/成就都不动"的
  正常区间，任何对浅层生存循环有意义的阈值都会把唯一能通关的 seed 判死。硬死锁
  已由 `STALL_ABORT_STEPS` 覆盖，故只调优先级、不放弃。同理推进优先必须是脉冲。
- **给补箭单独加 120 步窗口 + 200 步冷却**（"限制成本取代限制目标"）。8 局面板
  A/B 为净负：`defeat_gnome_warrior/3017` 从 done(1210 步, 15 箭) 退化成
  died(767 步, 4 箭)，通关 4→3。走到工作台、采料、被战斗打断都吃这个窗口，
  120 步往往在箭到手之前就用完。箭工厂改由推进优先模式兜（预算耗尽即关补箭）。

**A/B 实测**（`scripts/diag_rollout.py` 默认 8 局，每局 2000 步；`旧` = 同一份代码
把本轮机制逐项还原——HEAD 因缺 `energy_rate` 无法直接当基线）：

| 指标 | 旧 | 新（采用） | 新·带补箭窗口（已否） |
|---|---|---|---|
| 任务完成 | 4 | **4** | 3 |
| 死亡 | 3 | **2** | 3 |
| 超时 | 1 | 2 | 2 |
...
**结论要如实记**：这一轮在该面板上是**结果持平**（完成数 4→4、死亡 3→2、成就
143→141），不是提升。改动的价值在机制正确性与单局行为——同 seed/同任务的
`reach_floor_3/3017` 录制从"三次下楼各停 ~25 步、全程 0 石"变成"一次下楼、
L1 连续停留 210 步、带 8 木 + 8 石"，`ASCEND` 从主要动作里消失。代价是不再靠
"退回地表睡觉"续命，episode 从 1615 步缩到 952 步。

**仍未解决的瓶颈（下一轮的真问题）**：L1 清 8 怪所需火力仍不足——战斗模型自己算
`arrows_for_clear(L1)≈23`，而实际能带到 L1 的是 8 支左右；`reach_floor_3` 的
`task_progress` 仍是 0.125。这是"弹药/装备预算"问题，不是"推进逻辑"问题，
两条候选路线（提高可携带弹药 vs 改近战主战、资源投给甲/剑）需要独立 A/B。
...
### 6.2.7 流式无限世界（2026-08-20）

JAX 继续只计算固定 shape 的活动窗口；服务端以 `(seed, floor, chunk_x,
chunk_y)` 管理确定性 16×16 chunk。`SessionActor` 在玩家跨出 48×48 窗口时保存旧窗口
修改、加载相邻 chunk、重映射绝对坐标，并迁移活动窗口内的怪物/投射物/植物及
`mob_map` 占用缓存。`GET /map?window_size=N` 只返回玩家附近窗口，并附带
`map_origin`、`player_global_position`、`world_origin_chunk` 和 `loaded_chunks`。
窗口请求支持按需扩大（默认 80×80，规划失败时可继续请求更大的 chunk 拼接视图），
不把规划范围限制在活动 48×48 内。

每个 transition 的可选 `info` Parquet 扩展记录绝对坐标；跨窗口时额外写入
`world_transition`，便于视频、状态和世界扩张事件对齐。当前实现已打通坐标、chunk
缓存、边界换窗和实体迁移。新 chunk 已接入 Craftax 官方 smooth-world/dungeon 生成器，
并保留 NumPy 降级路径；ChunkStore 对离屏实体执行确定性随机游走、冷却衰减和有界
清理，梯子坐标以绝对坐标注册并提供最近梯子/跨 chunk 路由接口。录制器新增
`craftax.recording.continuous` 与 `scripts/concat_recording.py`，可按 `-segNNNN` 顺序
把多个 shard 的 MP4 合并为连续回放并生成 manifest。离屏 tick 是轻量 host 模拟，完整
战斗碰撞仍只在 JAX 活动窗口执行；这保持固定 shape，同时避免无限世界的内存失控。

规划器还维护当前子任务的绝对目标锁：资源、梯子和宝箱一旦选定，就以
`(floor, global_row, global_col)` 作为目标。chunk 换窗只重新计算到该目标的局部
BFS/滚动步，不重新选择同类目标；目标完成或连续确认不可达后才释放。默认 demo
观察窗口为 80×80（5×5 chunk），目标超出范围时可按需扩大窗口，因此换窗不会导致
任务目标被重新选择。
