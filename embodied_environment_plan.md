...
- 具身层（`SessionActor`）默认按 `contracts.DEFAULT_THIRST_RATE = 0.25` 覆盖
  （≈ 84 步/点，满水 ≈ 760 步），可由 `POST /sessions` 的 `thirst_rate` 显式改回 1.0。
...
| **0.25（具身层默认）** | 8/16 | **1** | 1297 | 18.6 | 12 | 2/16 |
...
- 默认取 0.25（长程任务优先，且这是唯一产出通关的一档）；改成 0.5 或 1.0 只需
