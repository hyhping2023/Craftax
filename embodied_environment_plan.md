...
已知瓶颈（游戏机制固有 + 当前执行器未闭合的部分）：
- **L0 表层制备生存墙（已量化确认）**：每个僵尸接战固定受击 1 次（2 伤，与剑无关——
  怪刷新即冷却<=0），制备装备 ~15-20 次采集 → ~10-15 次接战 → 20-30 伤 >> 9 血+被动
  回血；血 <8 无法安全睡（3.5x=7）。铁甲 1 件（10%）只把 2 伤降到 1.8，**不足以抵消
  接战次数**。1080 个 seed 无 L0 满铁甲（矿密度上限 ~8 铁，满甲需 12 铁）。
- **弓先制后的 L0 恢复环仍未闭合（2026-08 实测）**：弓先制能把玩家带到"弓+箭(5-8)+
  力量2(HP10)+木/石镐"的状态，且弓主动防御可维持 HP10 长时段（L0 采集 60+ 步无伤）。
  但死亡仍发生在 L0 恢复环：
  1) **睡眠 3.5x 打醒**：L0 怪在出生环 10-13 格持续刷新，安全点（>=14 格）睡眠也会在
     睡中被打醒（7 伤），HP>=8 时一睡掉 7 → 血线崩溃；
  2) **对角接近怪无法远射**：`_bow_combat` 只对同行/列怪提前射杀，对角接近的怪只能
     近身点射 → 每次接战 1 首击（2 伤），恢复速度（26 步/HP）追不上刷怪（0.07/回合）；
  3) **能量 0 + 血 <8 = 无恢复路径**：睡眠需血 >=8，REST 需能量 >0，两者都卡住时只能
     DO 等死（`_wait_action` 兜底）。
  结果：`enter_gnomish_mines` / `defeat_gnome_warrior` / `collect_diamond`（非 L0 钻石
  seed）仍全部失败；`collect_diamond` 仅在 L0 直接有钻石的 seed（2027/3017）通过；
  `enter_dungeon` 稳定通过。
- 真正出路（按可行性排序，下一阶段）：a) **角落/走廊防御位**——把回血/睡眠放在角落，
  怪只能沿直线接近 → 弓 14 格提前射杀 0 受击；b) **路线规划器**（§6.1.4 rolling
  planner 落地）：按周边方块分布规划采集路线，减少 L0 走动暴露；c) **清空出生环后
  短睡**——先射清 14 格内全部怪再睡，把睡中打醒窗口压到最短。
