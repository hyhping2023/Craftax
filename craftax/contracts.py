...
# 0.15 把它拉到约 140 步/点（满水约 1260 步），水仍是需要管理的资源，
# 但不会在长程录制中频繁打断战斗和下楼。RL 基准与 EnvParams() 默认值保持 1.0，
# 只有具身会话按此值覆盖。
DEFAULT_THIRST_RATE = 0.15

# 精力自然衰减倍率：具身长程任务使用原版的 1/4，减少频繁被迫睡眠，
# 同时保留 SLEEP 作为安全状态下的快速回血手段。
DEFAULT_ENERGY_RATE = 0.25

# 便携水契约：环境、规划器、服务摘要使用同一组单位。
# 一瓶水恢复 WATER_DRINK_AMOUNT 点 drink，水源旁可装到最大容量。
MAX_WATER_CANTEENS = 6
WATER_DRINK_AMOUNT = 4
